...
def run_experiment_for_dataset(dataset_path: str, dataset_name: str, result_subdir: str) -> None:
    ...
    result_dir = os.path.join('ISO_result—temp', result_subdir)
    ...
    for i in range(1, run_number_each_experiment + 1):
        ...
        population_history_file = os.path.join("population_history", f"ISOA_{dataset_name}_run_{i}_population_history.npz")
        # 保存为npz格式文件
        np.savez_compressed(
