...
if __name__ == '__main__':
    ...
    # 创建结果文件
    with open('Instance_results.txt', 'w', encoding='utf-8') as f:
        f.write("实验记录 - 5次运行结果\n")
        f.write("=" * 50 + "\n\n")
    ...
    for i in range(1, 5 + 1):
        ...
        from dataset.factoryDataset.Instance import Job_serial_number, Processing_time, J, J_num, M_num, O_num, kn, Special_Machine_ID

        result = run_single_experiment(i)
        results.append(result)

        # 将每次结果写入文件
        with open('Instance_results.txt', 'a', encoding='utf-8') as f:
            f.write(f"第 {i} 次运行结果:\n")
            f.write(f"最优适应度: \n{result['Optimal_fit']}\n")
    ...
    print("\n所有实验已完成，结果已保存到 Instance_results.txt 文件中")
