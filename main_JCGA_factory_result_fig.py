...
def Gantt(Machines, tn, gy_best, Job_serial_number, Special_Machine_ID) -> List[List[Any]]:
    ...
    for label, position in group_labels.items():
        plt.text(-3.3, position, label, fontsize=17, rotation=90, va='center', ha='right')

    # 在横轴上标出tn数组中的各个时刻，并画垂直虚线
    for t in tn:
        t_rounded = round(t, 2)
        plt.axvline(x=t_rounded, color='gray', linestyle='--', linewidth=0.8)
        plt.text(x=t_rounded + 0.2, y=-1.2, s=f'{t_rounded:.2f}', ha='center', va='top', fontsize=16)

    # 垂直实线，即最优适应度，makespan
    plt.axvline(x=gy_best, color='gray', linestyle='-', linewidth=0.8)
    plt.text(x=gy_best + 0.2, y=-1.2, s=f'{gy_best:.2f}', ha='center', va='top', fontsize=16)


    # plt.ylabel('Line', labelpad=20, fontsize=20)
    plt.xlabel('makespan (minute)', fontsize=20)
    plt.tight_layout()
    plt.savefig('factory_result_fig/JCGA_factory_result_fig/gantt.png', bbox_inches='tight')
    # plt.show()
...
def run_single_experiment(run_num: int, Job_serial_number: Dict[str, List[str]],
    ...
    for t in range(g.Max_Itertions):
        ...
        if best_individual_fitness < Optimal_fit:
            ...
            ans = Gantt(d.Machines, tn, round(Optimal_fit, 3), Job_serial_number, Special_Machine_ID)
    ...
    print("总配套关系为：", Matching_result_all_converted)
    print("配套时刻为：", tn)
    print("总装产线开始总装时间:", d.Machines[25].O_start)
    print("总装产线完成总装时间:", d.Machines[25].O_end)
    # 绘制适应度收敛图
...
def run_experiment_for_dataset(dataset_path: str, dataset_name: str, result_subdir: str) -> None:
    ...
    result_dir = os.path.join('JCGA_result3-temp', result_subdir)
...
if __name__ == '__main__':
    ...
    # DATASET_TYPE = 'randomDataset/big'
