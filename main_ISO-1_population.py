...
from ISO_1 import SO  # 导入此是仅改进种群初始化的
import os
import re
...
def run_experiment_for_dataset(dataset_path: str, dataset_name: str, result_subdir: str) -> None:
    ...
    result_dir = os.path.join('ISO_1_result-temp', result_subdir)
    os.makedirs(result_dir, exist_ok=True)
    result_file = os.path.join(result_dir, f"{dataset_name}_ISO_1_result.txt")
    results = []
    ...
    for i in range(1, run_number_each_experiment + 1):
        ...
        population_history_file = os.path.join("population_history", f"ISOA-1_{dataset_name}_run_{i}_population_history.npz")
        # 保存为npz格式文件
        np.savez_compressed(
