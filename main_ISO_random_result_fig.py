...
def Gantt(Machines, tn, gy_best, Job_serial_number, Special_Machine_ID) -> List[List[Any]]:
    ...
    machine_offset = {
        ...
        Special_Machine_ID["L2_pre_assembly_machine_ID_low"] + 2: 0.1,
        Special_Machine_ID["L2_last_machine_ID"] + 1: 0.1,
    ...
    for machine_index, Machine in enumerate(Machines):
        ...
        for task_index, (start, end) in enumerate(zip(start_times, end_times)):
            ...
            if machine_index in (Special_Machine_ID["L1_last_machine_ID"],
                ...
                b = 'I' + b[1:]
            # 成品产线最后一台机器标记为I前缀，成品F
            if machine_index == Special_Machine_ID["L4_last_machine_ID"]:
                b = 'F' + b[1:]
            ...
            plt.text(x=start + (end - start) / 2, y=adjusted_index,
                     s=b, va='center', ha='center',fontsize=14)
    ...
    # 1_small_SET1_L(10,7,4)_P(1,2,1) # yticklabels去看原始数据集excel大表
    # yticklabels = ['WD#1', 'WD#2', 'WD#3', 'DP#1', 'TP#1', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
    #                'AP#1', 'SR#1', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    # 2_small_SET2_L(10,7,4)_P(2,1,2)
    # yticklabels = ['WD#1', 'WD#2', 'WD#3', 'DP#1', 'TP#1', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
    #                'AP#1', 'SR#1', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    # 3_small_SET1_L(12,7,5)_P(2,1,2)
    # yticklabels = ['WD#1', 'WD#2', 'WD#3', 'WD#4', 'WD#5', 'DP#1', 'TP#1', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
    #                'AP#1', 'SR#1', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'TA#2', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    # 4_small_SET4_L(12,7,5)_P(1,4,2)
    # yticklabels = ['WD#1', 'WD#2', 'WD#3', 'WD#4', 'WD#5', 'DP#1', 'TP#1', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
    #                'AP#1', 'SR#1', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'TA#2', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    # 5_small_SET1_L(14,7,4)_P(3,3,2)
    # yticklabels = ['WD#1', 'WD#2', 'WD#3', 'WD#4', 'WD#5', 'DP#1', 'DP#2', 'TP#1', 'TP#2', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
    #                'AP#1', 'SR#1', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    # 6_small_SET1_L(14,7,4)_P(4,2,3)
    # yticklabels = ['WD#1', 'WD#2', 'WD#3', 'WD#4', 'WD#5', 'DP#1', 'DP#2', 'TP#1', 'TP#2', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
    #                'AP#1', 'SR#1', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    # 7_small_SET3_L(14,8,5)_P(3,4,2)
    # yticklabels = ['WD#1', 'WD#2', 'WD#3', 'WD#4', 'WD#5', 'DP#1', 'DP#2', 'TP#1', 'TP#2', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
    #                'AP#1', 'SR#1', 'SR#2', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'TA#2', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    # 8_small_SET4_L(14,8,5)_P(2,4,4)
    yticklabels = ['WD#1', 'WD#2', 'WD#3', 'WD#4', 'WD#5', 'DP#1', 'DP#2', 'TP#1', 'TP#2', 'ET#1', 'IW#1', 'MP#1', 'FT#1',
                   'AP#1', 'SR#1', 'SR#2', 'MR#1', 'PR#1', 'BA#1', 'ST#1', 'SA#1', 'MA#1', 'TA#1', 'TA#2', 'AS#1', 'AC#1', 'ED#1', 'FA#1']
    ...
    # 添加组标签
    group_labels = {
        'L1': (0 + Special_Machine_ID["L1_last_machine_ID"] + 1) / 2 - 0.5,
        'L2': (Special_Machine_ID["L1_last_machine_ID"] + 2 + Special_Machine_ID[
            "L2_last_machine_ID"] + 1) / 2 - 1 + group_spacing + 0.4,
        'L3': (Special_Machine_ID["L2_last_machine_ID"] + 2 + Special_Machine_ID[
            "L3_last_machine_ID"] + 1) / 2 + group_spacing + 0.5,
        'L4': (Special_Machine_ID["L4_last_machine_ID"] + 1 + Special_Machine_ID[
            "L4_last_machine_ID"] + 1) / 2 + 1 + group_spacing + 0.3
    }

    # 绘制组标签
    for label, position in group_labels.items():
        plt.text(-3.7, position, label, fontsize=17, rotation=90, va='center', ha='right')

    # 在横轴上标出tn数组中的各个时刻，并画垂直虚线
    for t in tn:
        t_rounded = round(t, 2)
        plt.axvline(x=t_rounded, color='gray', linestyle='--', linewidth=0.8)
        plt.text(x=t_rounded + 0.2, y=-1.1, s=f'{t_rounded:.2f}', ha='center', va='top', fontsize=16)

    # 垂直实线，即最优适应度，makespan
    plt.axvline(x=gy_best, color='gray', linestyle='-', linewidth=0.8)
    plt.text(x=gy_best + 0.2, y=-1.1, s=f'{gy_best:.2f}', ha='center', va='top', fontsize=16)


    # plt.ylabel('Line', labelpad=20, fontsize=20)
    plt.xlabel('makespan (minute)', fontsize=20)
    plt.tight_layout()
    plt.savefig('random_result_fig/ISO_random_result_fig/gantt.png', bbox_inches='tight')
    # plt.show()
...
def run_single_experiment(run_num: int, Job_serial_number: Dict[str, List[str]],
    ...
    for t in range(1, s.Max_Itertions + 1):   # t从1开始取值
        ...
        if gy_best < Optimal_fit:
            ...
            ans = Gantt(d.Machines, tn, round(gy_best, 3), Job_serial_number, Special_Machine_ID)
...
def run_experiment_for_dataset(dataset_path: str, dataset_name: str, result_subdir: str) -> None:
    ...
    result_dir = os.path.join('ISO_result3-temp', result_subdir)
