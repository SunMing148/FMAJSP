...
def main():
    # 设置文件夹路径和Excel文件路径
    base_folder = r'result\MWOA_result_redo\randomDatasetResult\big'
    ...
    for set_num in range(1, 6):
        ...
        for txt_file in txt_files:
            match = re.search(r'Instance_L\((\d+,\d+,\d+)\)_P\((\d+,\d+,\d+)\)_MWOA_result\.txt', txt_file)
            if match:
...
############################################不同算法改这里
                ...
                for j, value in enumerate(data):
                    cell = f'T{data_start_row + j}'
                    ws[cell] = value

                print(f"已处理: {cell_value} -> T{data_start_row}:T{data_start_row + 9}")
############################################
            except Exception as e:
