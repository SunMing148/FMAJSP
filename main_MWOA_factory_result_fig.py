...
def Gantt(Machines, tn, gy_best, Job_serial_number, Special_Machine_ID) -> List[List[Any]]:
    ...
    plt.tight_layout()
    plt.savefig('factory_result_fig/MWOA_factory_result_fig/gantt.png', bbox_inches='tight')
    # plt.show()
...
def run_single_experiment(run_num: int, Job_serial_number: Dict[str, List[str]],
    ...
    for t in range(w.Max_Itertions):      # 保证t从0开始取值
        ...
        if Leader_score < Optimal_fit:
            ...
            ans = Gantt(d.Machines, tn, round(Leader_score, 3), Job_serial_number, Special_Machine_ID)
...
def run_experiment_for_dataset(dataset_path: str, dataset_name: str, result_subdir: str) -> None:
    ...
    result_dir = os.path.join('MWOA_result3-temp', result_subdir)
